...
import { Heart, MessageCircle, Share2, ArrowLeft, Loader2, Check, ChevronLeft, ChevronRight, MoreHorizontal, Pencil, Trash2 } from 'lucide-react';
...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeletePost = async () => {
    ...
    const success = await deleteFeedPost(id);
    if (success) {
      router.push('/');
    } else {
  ...
  const handleSaveEdit = async () => {
    ...
    setIsUpdatingPost(true);
    const success = await updateFeedPost(id, editContent);
    if (success) {
      setPost(p => p ? { ...p, content: editContent } : null);
              ...
              {user?.id === post.author?.id && (
                  <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                          <button className="h-8 w-8 text-muted-foreground hover:bg-muted hover:text-foreground rounded-full flex items-center justify-center transition-colors" disabled={isUpdatingPost}>
                              {isUpdatingPost ? <Loader2 size={16} className="animate-spin text-amber-500" /> : <MoreHorizontal size={16} />}
                          </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-40 z-50">
                          ...
                          <DropdownMenuItem onClick={handleDeletePost} className="text-destructive focus:bg-destructive focus:text-destructive-foreground cursor-pointer">
          ...
          {editingPost ? (
              <div className="mb-4 space-y-3">
                  <textarea 
                      ...
                      className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-base text-foreground resize-y focus:outline-none focus:ring-2 focus:ring-amber-400 min-h-[120px]"
                      autoFocus
                  />
                  <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setEditingPost(false)} disabled={isUpdatingPost} className="text-sm px-3 py-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors font-medium">
                          Cancel
                      </button>
                      <button onClick={handleSaveEdit} disabled={isUpdatingPost} className="text-sm px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white transition-colors font-medium flex items-center gap-1">
              ...
              <p className="text-base text-foreground/85 leading-relaxed mb-4 whitespace-pre-wrap">{post.content}</p>
          )}
        </div>
