...
export function useGroupChat(groupId: string | null) {
    ...
    const loadMessages = useCallback(async (gid: string) => {
        const msgs = await getGroupMessages(gid);
        setMessages(msgs);
    }, []);
    ...
    const send = useCallback(async (content: string) => {
        ...
        setSending(true);
        try {
            await sendGroupMessage(groupId, user.id, content.trim());
    ...
    return { messages, members, myRole, isMember, loading, sending, send, join, leave, refreshMembers };
}
