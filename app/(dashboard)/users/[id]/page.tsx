...
export default function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-start gap-6 md:gap-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              {isOwnProfile
                ? (
                  <button onClick={() => setShowEditModal(true)} className="flex items-center gap-1.5 border border-border text-sm font-medium px-3.5 py-1.5 rounded-lg hover:bg-accent transition-colors no-underline text-foreground">
      ...
      {showEditModal && (
        <div 
          className="fixed inset-0 z-[60] bg-black/70 flex items-start justify-center overflow-y-auto p-4 md:py-10 backdrop-blur-sm"
          onClick={() => setShowEditModal(false)}
        >
          <div 
            className="bg-card border border-border rounded-xl w-full max-w-4xl shadow-2xl overflow-hidden mt-10 md:mt-0"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-border bg-muted/30">
              <h2 className="font-semibold text-lg">Edit Profile</h2>
              <button onClick={() => setShowEditModal(false)} className="p-1.5 rounded-lg hover:bg-accent text-muted-foreground transition-colors">
                <Settings size={18} className="rotate-45 opacity-0 absolute" />{/* just for spacing layout if needed */}
