...
export function MarketplaceDetail() {
    ...
    const handleSendInquiry = async () => {
        ...
        try {
            ...
            const contextContent = `Inquiry regarding Marketplace item: "${item.title}"\n🔗 ${window.location.origin}/marketplace/${item.id}`;
            const { data: contextMsg } = await db.from('messages').insert({
            ...
            await db.from('messages').insert({
                ...
                receiver_id: item.sellerId,
                content,
                reply_to_id: contextMsg?.id || null,
            });
    ...
    return (
        <div className="max-w-5xl mx-auto py-4 md:py-8 animate-fade-in space-y-4 md:space-y-6">
            <Button variant="ghost" size="sm" asChild className="mb-2 text-muted-foreground hover:text-foreground md:-ml-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start">
                {/* Image Section */}
                <GlassSurface className={cn("overflow-hidden flex items-center justify-center bg-zinc-100 dark:bg-zinc-900 min-h-[250px] md:min-h-[350px]", !isAvailable && "opacity-80 grayscale-[0.5]")}>
                    {imageUrl ? (
                ...
                <div className="space-y-6">
                    ...
                    {/* Seller Card + Inquiry */}
                    <GlassSurface className="p-4 md:p-5 space-y-4 border-accent-gold/20">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            ...
                            {!isOwner && isAvailable && item.sellerId && !sent && (
                                <Button
                                    ...
                                    className={cn("gap-2 shadow-md transition-all w-full sm:w-auto", showInquiry ? "bg-zinc-700 hover:bg-zinc-800 text-white" : "bg-amber-500 hover:bg-amber-600 text-white")}
                                >
                    ...
                    {isOwner && isAvailable && (
                        <div className="flex flex-col sm:flex-row flex-wrap gap-3 pt-4 border-t border-border">
                            <Button onClick={handleMarkSold} className="bg-red-600 hover:bg-red-700 text-white flex-1 w-full sm:w-auto">
                                <CheckCircle2 className="w-4 h-4 mr-2" /> Mark as Sold
                            </Button>
                            <Button onClick={handleMarkReserved} variant="outline" className="flex-1 w-full sm:w-auto">
                                <Package className="w-4 h-4 mr-2" /> Reserve
                            </Button>
                            <Button onClick={handleCancel} variant="ghost" className="text-red-500 hover:text-red-700 flex-1 w-full sm:w-auto">
